...
import { Plus, Trash, Edit, Loader2, Upload, X, ImageIcon } from "lucide-react";
...
const BLOG_FEATURED_IMAGE_MAX_BYTES = BLOG_FEATURED_IMAGE_MAX_MB * 1024 * 1024;
...
const AdminBlog = () => {
    ...
    const fetchPosts = async () => {
        setIsLoading(true);
        try {
            const res = await api.get("/api/blog/admin/list");
    ...
    const handleFeaturedChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.size > BLOG_FEATURED_IMAGE_MAX_BYTES) {
            toast.error(`Image is too large. Maximum size is ${BLOG_FEATURED_IMAGE_MAX_MB} MB.`);
            e.target.value = "";
            return;
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (featuredFile && featuredFile.size > BLOG_FEATURED_IMAGE_MAX_BYTES) {
            toast.error(`Image is too large. Maximum size is ${BLOG_FEATURED_IMAGE_MAX_MB} MB.`);
            return;
        }
        ...
        try {
            ...
            if (editingId) {
                await api.put(`/api/blog/admin/${editingId}`, fd, {
                    headers: { "Content-Type": "multipart/form-data" },
                });
                toast.success("Post updated");
            } else {
                await api.post("/api/blog/admin/create", fd, {
                    headers: { "Content-Type": "multipart/form-data" },
                });
        ...
        <div className="p-8 max-w-7xl mx-auto space-y-8">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Manage articles for SEO and news section.</p>
                </div>
            </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-2">
                                ...
                                />
                                <p className="text-xs text-muted-foreground">Used in /blog/:slug. Leave blank to auto-generate from title.</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-muted-foreground mb-1">Max size: {BLOG_FEATURED_IMAGE_MAX_MB} MB. JPG, PNG or WebP recommended.</p>
                            <div className="flex items-center gap-4 flex-wrap">
                                <Label className="cursor-pointer flex items-center gap-2 text-primary hover:underline">
                                    <Upload className="w-4 h-4" />
                                    Upload image (max {BLOG_FEATURED_IMAGE_MAX_MB} MB)
                                    <input
                                        type="file"
                                        accept="image/*"
                                        className="hidden"
                        ...
                        <div className="flex flex-wrap gap-6">
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                />
                                <Label htmlFor="isPublished">Published (visible on /blog)</Label>
                            </div>
                        </div>
            ...
            <Card>
                ...
                <CardContent>
                    {isLoading ? (
                        ...
                        <p className="text-center text-muted-foreground py-8">No posts yet. Create one above.</p>
                    ) : (
                        <Table>
                            <TableHeader>
                                <TableRow>
