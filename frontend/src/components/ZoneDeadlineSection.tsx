        ...
        apiClient.get("/api/zone-deadline")
            .then(res => {
                if (res.data.success && res.data.data) {
                    ...
                    if (s.stat1Value) setStat1Value(s.stat1Value);
                    if (s.stat1Label) setStat1Label(s.stat1Label);
                    if (s.stat2Label) setStat2Label(s.stat2Label);
                    if (s.stat3Value) setStat3Value(s.stat3Value);
                    if (s.stat3Label) setStat3Label(s.stat3Label);
                    if (s.ctaLine1 !== undefined) setCtaLine1(s.ctaLine1);
                    if (s.ctaLine2 !== undefined) setCtaLine2(s.ctaLine2);
                    if (s.buttonText) setButtonText(s.buttonText);
                    if (s.countdownTargetDate) setTargetDate(new Date(s.countdownTargetDate).getTime());
                }
            })
            .catch(() => {});
    }, []);
